...
def plot(records: list[MeasurementRecord], out_path: str, agg: str, compare_n: Optional[int]) -> Optional[str]:
    ...
    for rec in records:
        if compare_n is not None and rec.n != compare_n:
            continue
        if rec.average_watts is not None and rec.average_watts >= 0:
            power_by_source[rec.source].append(rec.average_watts)
    ...
    style_axes(ax, "Average Power by Source", "Source", "Power (W)")
    fig.tight_layout()

...
def plot(records: list[CaseRecord], out_path: str, agg: str, error_bars: str) -> Optional[list[str]]:
    ...
    for rec in records:
        if rec.backend in {"", "gt"} or rec.algorithm not in {"bitonic", "map_reduce"} or rec.n is None or rec.k is None: continue
    ...
    for k, backend_map in sorted(grouped.items()):
        for backend, algo_map in sorted(backend_map.items()):
            ...
            for algorithm in ("bitonic", "map_reduce"):
                if algorithm not in algo_map: continue
                ...
                if xs:
                    ...
                    if error_bars != "none": ax.fill_between(xs, lowers, uppers, alpha=0.15, color=line.get_color())
            ...
            ax.set_yscale("log")
            style_axes(ax, f"Algorithmic Runtime ({backend}): bitonic vs map_reduce (K = {k})", "N (log2 scale)", "Time (ms, log scale)")

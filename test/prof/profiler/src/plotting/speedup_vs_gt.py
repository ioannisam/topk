...
def plot(records: list[CaseRecord], out_path: str, agg: str) -> Optional[list[str]]:
    ...
    # series: backend -> k -> label -> n -> list[speedup]
    ...
    for rec in records:
        ...
        if gt_t_algo is not None and t_algo is not None and t_algo > 0:
            ...
            if math.isfinite(speedup) and speedup > 0:
                series[rec.backend][rec.k][rec.algorithm][rec.n].append(speedup)
    ...
    for backend, k_map in sorted(series.items()):
        for k, label_map in sorted(k_map.items()):
            ...
            style_axes(ax, f"Algorithmic Speedup vs GT ({backend.upper()}, K = {k})", "N (log2 scale)", "Speedup")

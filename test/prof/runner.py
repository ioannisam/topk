...
def parse_args():
    ...
    parser.add_argument("--types", nargs="*", default=["double", "float", "fp16", "int", "uint"], help="Datatypes to test")
    parser.add_argument("--energy", choices=["none", "auto", "rapl", "gpu"], default="none", help="Energy measurement wrapper")
...
def main():
    ...
    with open(args.output_raw, "w", encoding="utf-8") as f_raw:
        ...
        f_raw.write(f"Defaults: k=min({args.k},N) mode={args.mode} run={args.run} threads={args.threads} verify={args.verify}\n")

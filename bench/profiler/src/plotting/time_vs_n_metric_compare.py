...
def plot(records: list[CaseRecord], out_path: str, agg: str, error_bars: str) -> Optional[list[str]]:
    ...
    for rec in records:
        if rec.n is None or not rec.backend or rec.k is None:
            continue
        for metric in ("algorithmic", "end-to-end"):
            time_ms = select_time_ms(rec, metric)
            if time_ms is None or time_ms <= 0:
                continue
            grouped[rec.k][rec.backend][rec.algorithm][metric][rec.n].append(time_ms)
    ...
    for k, backend_map in sorted(grouped.items()):
        for backend, algo_map in sorted(backend_map.items()):
            ...
            for algorithm, metric_map in sorted(algo_map.items()):
                ...
                if algo_series:
                    ...
                    for n in xs:
                        ...
                        uppers.append(hi)
                    (line,) = ax.plot(
                        xs, ys, **time_metric_style("algorithmic", label=algorithm if algorithm else backend)
                    )
            ...
            style_axes(
                ax,
                f"Metrics Comparison ({backend}, K = {k}): Algorithmic vs End-to-End",
                "N (log2 scale)",
                "Time (ms, log scale)",
            )

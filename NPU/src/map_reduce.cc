...
void map_reduce_step_kernel(int32_t* restrict in_buf, int32_t* restrict out_buf, int32_t* restrict cfg_buf) {
    ...
    int32_t sentinel_val = cfg_buf[2];
    ...
    aie::vector<int32_t, vector_width> sentinel = aie::broadcast<int32_t, vector_width>(sentinel_val);
    ...
    for (int i = 0; i < total_elements; i += vector_width) {
        ...
        aie::vector<int32_t, vector_width> out_v = aie::select(sentinel, v, cmp_mask);

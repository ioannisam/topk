...
template <typename T>
struct SimdTraits512;
template <typename T>
struct SimdTraits256;
...
struct SimdTraits512<float> {
	...
	template <int J>
	static Mask get_blend_mask(std::size_t i, std::size_t k) {
	...
	static std::uint64_t get_candidate_mask(const float* ptr, float threshold) {
...
template <>
struct SimdTraits512<std::int32_t> {
	using Vec = __m512i;
	using Mask = __mmask16;
	static constexpr std::size_t width = 16;
	...
	// Map-Reduce Primitives
	template <bool WantMax>
	static std::uint64_t get_candidate_mask(const std::int32_t* ptr, std::int32_t threshold) {
...
template <>
struct SimdTraits512<std::uint32_t> {
	using Vec = __m512i;
	using Mask = __mmask16;
	static constexpr std::size_t width = 16;
	...
	// Map-Reduce Primitives
	template <bool WantMax>
	static std::uint64_t get_candidate_mask(const std::uint32_t* ptr, std::uint32_t threshold) {
		const __m512i v = _mm512_loadu_si512(reinterpret_cast<const __m512i*>(ptr));
...
struct SimdTraits512<double> {
	...
	// Map-Reduce Primitives
	template <bool WantMax>
	static std::uint64_t get_candidate_mask(const double* ptr, double threshold) {
...
struct SimdTraits512<_Float16> {
	...
	// Map-Reduce Primitives
	template <bool WantMax>
	static std::uint64_t get_candidate_mask(const _Float16* ptr, _Float16 threshold) {
...
template <>
struct SimdTraits256<std::int32_t> {
	using Vec = __m256i;
	using Mask = __m256i;
	static constexpr std::size_t width = 8;
...
template <>
struct SimdTraits256<std::uint32_t> {
	using Vec = __m256i;
	using Mask = __m256i;
	static constexpr std::size_t width = 8;
...
template <typename T>
inline std::size_t simd_block_width(bool use_512, bool use_256) {
	if (use_512) {
		if constexpr (has_simd512_width<T>::value) {
...
template <bool WantMax, typename T>
inline std::uint64_t get_candidate_mask_simd(const T*, T, bool, bool) {
	return 0;
}
template <typename T>
inline std::size_t simd_block_width(bool, bool) {
	return 1;
}
#endif

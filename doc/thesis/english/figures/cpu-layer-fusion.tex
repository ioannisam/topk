\begin{figure}[H]
    ...
    \begin{tikzpicture}[x=1.28cm, y=1.0cm]
        ...
        \node[panel] at (-1.75,1.30) {(a) naive transfer};

        \foreach \i/\lab in {0/{$2^{20}$},1/{$2^{19}$},2/{$\cdots$},3/{$32$},4/{$16$},5/{$8$},6/{$4$},7/{$2$},8/{$1$}} {
            \node[lane, minimum width=1.02cm] (a\i) at (\i,0.35) {\lab};
        }
        \node[rowlab] at (a0.west) {layer ($j$)};

        \node[blkdrm, minimum width=11.7cm, minimum height=0.46cm] (adram) at (4,-1.10) {main memory};
        \foreach \i in {0,...,8} {
        ...
        \node[note, anchor=north] at (4,-1.48) {nine passes, $2n$ each};

        \begin{scope}[yshift=-4.05cm]

            % (b) fused: layers grouped by distance j into a single pass
            \node[panel] at (-1.75,1.62) {(b) layer fusion};
            ...
            \node[rowlab] at (b0.west) {layer ($j$)};

            \draw[brace] (-0.44,0.66) -- (2.44,0.66);
            \draw[brace] (2.56,0.66) -- (3.44,0.66);
            \draw[brace] (3.56,0.66) -- (8.44,0.66);
            \node[notemute, font=\tiny] at (1.0,1.02) {$j>$ segment};
            \node[notemute, font=\tiny] at (3.0,1.02) {$j\le$ segment};
            \node[notemute, font=\tiny] at (6.0,1.02) {$j<W$};

            \node[blkdrm, minimum width=11.7cm, minimum height=0.46cm] (bdram) at (4,-1.10) {main memory};
            ...
            \node[blkcac, minimum width=1.15cm, minimum height=0.40cm] at (3,-0.42) {\tiny segment};
            \node[blkreg, minimum width=1.35cm, minimum height=0.40cm] at (6,-0.42) {\tiny reg.};

            \node[note, anchor=north] at (4,-1.48) {five passes};
        \end{scope}

    \end{tikzpicture}
    \caption[Fusing consecutive layers into one pass]{The fusion of consecutive layers into a single
    pass; each pair of arrows is one read and one write of the entire array. In (a)
    each layer is a separate pass to memory; in (b) the layers are grouped by distance
    $j$ and only the last group retains a separate pass. The numbers in the figure
    are illustrative.}

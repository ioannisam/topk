...
the user, preventing the system from being left in a pinned state. The failure
of an individual sweep does not stop the rest; it is simply logged as a
warning.
...
\begin{table}[H]
    ...
    \begin{tabular}{@{}lp{2.9cm}lp{2.6cm}p{4.0cm}@{}}
        ...
        \hline
        q & integer 0--63 & --- & 3--24 & Input size $n = 2^q$ (the only mandatory one) \\
        ...
        dist & \mbox{uniform} $|$ \mbox{normal} $|$ \mbox{trimodal} $|$ \mbox{sorted} $|$ \mbox{reverse} $|$ \mbox{adversarial} & uniform & uniform (all five in the distribution sweep) & Input distribution \\
        ...
        verify & true $|$ false & false & true & Check against the verification implementation \\
...
The --plot argument selects the plots and --input the sweep (the default is the
timing sweep). Because the other plots use their own files, the combined
targets are recommended, which run the measurement and the generation directly:
...
Two parameters determine the presentation. --energy-metric selects between
gross and net energy. The sweeps of this work use the net figure
(section~\ref{sec:meth-energy}). The --error-bars parameter sets the dispersion
band (standard deviation, deciles or none). The band is drawn only if there are
at least two samples per point, that is, in the distribution sweep (two seeds)
and the energy sweep (three repetitions). In the automated run, the bands are
disabled entirely.
